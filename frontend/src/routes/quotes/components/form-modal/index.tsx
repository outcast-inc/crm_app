...
export const QuotesFormModal: FC<Props> = ({
    ...
    const {
        ...
        queryResult: { isLoading: isLoadingCompanies, data: companyData },
    } = useCompaniesSelect();

    const {
        selectProps: selectPropsContacts,
        queryResult: { isLoading: isLoadingContact, data: contactData },
    } = useContactsSelect();
    
      const {
        selectProps: selectPropsSalesOwners,
        queryResult: { isLoading: isLoadingSalesOwners, data: userData },
    } = useUsersSelect();
                ...
                <Form {...formProps} layout="vertical">
                    ...
                    >
                        <Select 
                            ...
                            options={
                                userData?.data.map(({ _id, name, avatarUrl }) => ({
                                    value: _id,
                                    label: name
                            })) ?? []}
                        />
                    ...
                    <Form.Item
                        ...
                        initialValue={
                            ...
                            formProps?.initialValues?.company?._id
                        }
                        label="Company"
                        extra={
                            <Button
                    ...
                    <Form.Item
                        rules={[{ required: true }]}
                        name={["contactId"]}
                        initialValue={formProps?.initialValues?.contact?._id}
                        label="Quote Contact"
                    >
                        <Select 
                            placeholder="Please select contact"
                            options={
                                contactData?.data.map(({ _id, name }) => ({
                                    value: _id,
                                    label: name
                            })) ?? []}
                        />
