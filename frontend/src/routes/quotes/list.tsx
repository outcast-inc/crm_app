...
export const QuotesListPage: FC<PropsWithChildren> = ({ children }) => {
    ...
    const { selectProps: selectPropsCompanies, queryResult: companyQueryResult } = useCompaniesSelect();

    const { selectProps: selectPropsUsers, queryResult: userQueryResult } = useUsersSelect();
        ...
        <div className="page-container">
            ...
            >
                ...
                >
                    ...
                    />
                    <Table.Column<Quote> 
                        dataIndex={["company", "_id"]}
                        title="Company"
                        defaultFilteredValue={getDefaultFilter("company._id", filters, "in")}
                        filterDropdown={(props) => (
                            <FilterDropdown {...props}>
                                <Select 
                                    ...
                                    style={{ width: 220 }}
                                    options={
                                        companyQueryResult?.data?.data.map(({ _id, name }) => ({
                                            value: _id,
                                            label: name
                                    })) ?? []}
                                />
                    ...
                    <Table.Column<Quote>
                        ...
                        return (
                            ...
                            <Select
                                ...
                                options={
                                    userQueryResult?.data?.data.map(({ _id, name }) => ({
                                        value: _id,
                                        label: name
                                })) ?? []}
                            />
