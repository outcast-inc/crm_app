...
export const CompaniesTableView: FC<Props> = ({ tableProps, filters, sorters }) => {

    const { selectProps: selectPropsUsers, queryResult: queryPropsUsers } = useUsersSelect();
    const { selectProps: selectPropsContacts, queryResult: queryPropsContacts } = useContactsSelect();
    ...
    return (
        ...
        >
            ...
            />
            <Table.Column<Company> 
                dataIndex={["salesOwner", "_id"]}
                title="Sales Owner"
                defaultFilteredValue={getDefaultFilter("salesOwner._id", filters)}
                filterDropdown={(props) => (
                    <FilterDropdown {...props}>
                        <Select 
            ...
            />
            <Table.Column<Company> 
                dataIndex={["contacts", "_id"]}
                title="Related Contacts"
                defaultFilteredValue={getDefaultFilter("contacts._id", filters, "in")}
                filterDropdown={(props) => (
                    <FilterDropdown {...props}>
                        <Select 
                            mode="multiple"
                            ...
                            style={{ width: 220 }}
                            options={
                                queryPropsContacts?.data?.data?.map(
                                  ({ _id, name }) => ({
                                    value: _id,
                                    label: name,
                                })) ?? []
