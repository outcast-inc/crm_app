...
export const SalesEditPage = () => {
    ...
    const { selectProps: stageSelectProps, queryResult: stageQueryResult } = useDealStagesSelect();
    ...
    useEffect(() => {
        if (deal?.company?._id !== companyIdField) {
        ...
        if (hasContact) {
            ...
            return (
                <Form.Item
                    ...
                    name="dealContactId"
                    rules={[{ required: true }]}
                    initialValue={deal?.dealContact?._id}
                    ...
                    <Col span={12}>
                        <Form.Item label="Stage" name="stageId">
                            <Select 
                                ...
                                showSearch={false}
                                options={
                                    stageQueryResult.data?.data?.map((stage) => ({
                                        label: stage.title,
                                        value: stage._id
                                    })).concat({
                                        label: "UNASSIGNED",
